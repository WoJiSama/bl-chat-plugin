...
export class GlobalStyleLearnerManager {
  ...
  readMemory(config = {}) {
    ...
    try {
      if (fs.existsSync(file)) {
        ...
        this.memory = data && typeof data === "object" ? { ...createEmptyMemory(), ...data } : createEmptyMemory()
        return this.memory
      }
    } catch (error) {
  ...
  buildPrompt(config = {}) {
    ...
    if (!essence.length && !dross.length) return ""

    const lines = [
      "【希洛全局表达学习】",
      "这是从多个群的离散特征里沉淀出的表达策略，不是模仿任何具体群友。回复时自然吸收精华、避开糟粕，不要提到学习过程。"
    ]
    for (const item of essence) lines.push(`- 可吸收：${item.rule}`)
    for (const item of dross) lines.push(`- 避免：${item.rule}`)
    return lines.join("\n")
  }
  ...
  buildStatus(config = {}) {
    ...
    return [
      ...
      `当前可用策略：${essenceCount} 条；避坑规则：${drossCount} 条`,
      enough
        ? "现在会把少量高权重表达策略注入回复。"
        : "现在只做离散统计，还不会影响回复。"
    ].join("\n")
